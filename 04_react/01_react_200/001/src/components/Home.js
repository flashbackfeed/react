...
function Home() {
  ...
  // 변수값 + html 태그를 같이 사용이 가능함(JSX 표현식)
...
// TODO: 웹 브라우저 화면에 표시되는 영역
  return (
    ...
    <div>
      ...
      {/* 8) html 태그 속성의 값으로 {} 넣어보기 : O */}
      <a href={'http://www.naver.com'}>네이버</a><br/>
      <button onClick={myClick}>클릭하세요</button><br/>

      {/* 9) 자바스크립트 내장 함수 : O */}
      {console.log("이것도 출력될까요?")}<br/>
      {/* 오늘 요일(1~7) : new Date() */}
      {new Date().getDay()}<br/>
      {new Date().getFullYear() + "년"}<br/>
      {(new Date().getMonth()+1) + "월"}<br/>
      {new Date().getDate() + "일"}<br/>
